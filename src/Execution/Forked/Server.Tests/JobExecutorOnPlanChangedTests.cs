...
namespace Jobbr.Server.ForkedExecution.Tests
{
    ...
    public class JobExecutorOnPlanChangedTests : TestBase
    {
        ...
        public void StartEmpty_GetsNewPlanWithOneJob_ExecutesJob()
        {
            ...
            // Wait
            ChannelStore.WaitForStatusUpdate(updatesFromAllJobs => updatesFromAllJobs[fakeJobRun.Id].Contains(JobRunStates.Starting), 3000);
        ...
        public void StartEmpty_GetsMultipleJobs_ExecutesMultipleJobs()
        {
            ...
            ChannelStore.WaitForStatusUpdate(updatesFromAllJobs => updatesFromAllJobs.Count == 3 && updatesFromAllJobs.All(kvp => kvp.Value.Contains(JobRunStates.Starting)), 3000);

            // Test
            var allStatesForJob1 = ChannelStore.AllStatusUpdates[fakeJobRun1.Id];
            var allStatesForJob2 = ChannelStore.AllStatusUpdates[fakeJobRun2.Id];
            var allStatesForJob3 = ChannelStore.AllStatusUpdates[fakeJobRun3.Id];

            Assert.AreEqual(3, RunContextMockFactory.Count, "The factory should have created only the requested amount of contexts");
        ...
        public void ContainsAItemInPlan_GetsAdditional_ExecutesMultipleJobs()
        {
            ...
            // Wait
            var didStart2Jobs = ChannelStore.WaitForStatusUpdate(updatesFromAllJobs => updatesFromAllJobs.Count == 2 && updatesFromAllJobs.All(kvp => kvp.Value.Contains(JobRunStates.Starting) && kvp.Value.Count == 1), 5000);

            // Test
            var statesPerJobRun = string.Join("\n", ChannelStore.AllStatusUpdates.Select(u => $"- JobRun #{u.Key}, States: {string.Join(",", u.Value)}"));
        ...
        public void StartDateInFuture_GetsChangedToNow_ShouldExecute()
        {
            ...
            // Wait
            var didStartJob = ChannelStore.WaitForStatusUpdate(updatesFromAllJobs => updatesFromAllJobs[fakeJobRun1.Id].Contains(JobRunStates.Starting), 3000);
        ...
        public void StartDateInFuture_MoveToFuture_ShouldExecute()
        {
            ...
            TimeProvider.Set(DateTime.UtcNow.AddDays(1));

            // Force re-evaluation
            TimerMock.CallbackOnce();
        ...
        public void LimitTo2JobsAndStartWith2_GetsUpdatedPlanWith3Jobs_DoesNotStartThird()
        {
            ...
            // Wait
            ChannelStore.WaitForStatusUpdate(updatesFromAllJobs => updatesFromAllJobs[fakeJobRun1.Id].Count + updatesFromAllJobs[fakeJobRun2.Id].Count == 2, 3000);

            // Test
            Assert.IsFalse(ChannelStore.AllStatusUpdates.ContainsKey(fakeJobRun3.Id), "There should be no updates for the third job");
        }
        ...
        public void LimitTo2JobsAndStartWith2_GetsUpdatedPlanWith3Jobs_StartsThirdIfCapacityIsAvailable()
        {
            ...
            RunContextMockFactory[fakeJobRun1.Id].RaiseEnded();

            // Simulate timer
            TimerMock.CallbackOnce();

            // Wait (optional)
            ChannelStore.WaitForStatusUpdate(updatesFromAllJobs => updatesFromAllJobs.Count == 3, 3000);

            // Test
            Assert.AreEqual(3, RunContextMockFactory.Count, "The third job should have been started after the first one has ended");
        }
        ...
        public void StartOverbooked_ReducedPlan_DoesNotStartRemoved()
        {
            ...
            Assert.AreEqual(2, RunContextMockFactory.Count);
        }
    }
}