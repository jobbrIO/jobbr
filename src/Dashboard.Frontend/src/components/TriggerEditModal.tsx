...
const TriggerEditModal = ({ 
  ...
  const loadTriggerData = async () => {
    ...
    try {
      ...
      const triggerResult = await apiClient.getTrigger(jobId, triggerId);
      setTrigger(triggerResult);
      
      // Populate form fields
      setTriggerType(triggerResult.triggerType);
      setIsActive(triggerResult.isActive);
      setComment(triggerResult.comment || '');
      setUserId(triggerResult.userId || '');
      setUserDisplayName(triggerResult.userDisplayName || '');
      setParameters(triggerResult.parameters ? renderParameter(triggerResult.parameters) : '{}');
      
      // Type-specific fields
      if (triggerResult.triggerType === 'Recurring') {
        setDefinition(triggerResult.definition || '');
        setStartDateTimeUtc(triggerResult.startDateTimeUtc ? triggerResult.startDateTimeUtc.slice(0, 16) : '');
        setEndDateTimeUtc(triggerResult.endDateTimeUtc ? triggerResult.endDateTimeUtc.slice(0, 16) : '');
      } else if (triggerResult.triggerType === 'Scheduled') {
        setStartDateTimeUtc(triggerResult.startDateTimeUtc ? triggerResult.startDateTimeUtc.slice(0, 16) : '');
      } else if (triggerResult.triggerType === 'Instant') {
        setDelayedMinutes(triggerResult.delayedMinutes || 0);
      }
    } catch (err) {
      setError(`Error loading trigger details${err instanceof Error ? `: ${err.message}` : ''}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await apiClient.updateTrigger(updatedTrigger, jobId);
      ...
      <Modal.Body>
        {loading ? (
          ...
          <Alert variant="danger">
            ...
            <p>{error}</p>
            <Button variant="outline-danger" onClick={loadTriggerData}>
              Try Again
            </Button>
          </Alert>
        ) : !trigger ? (
