...
const TriggerCreateModal = ({ 
  ...
  const loadJobDetails = async () => {
    try {
      ...
      const jobResult = await apiClient.getJob(jobId);
      setJob(jobResult);
      ...
      setError(`Error loading job details${err instanceof Error ? `: ${err.message}` : ''}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const resetForm = () => {
    ...
    // Reset start time to 5 minutes from now
    const now = new Date();
  ...
  const handleSave = async () => {
    try {
      ...
      await apiClient.createTrigger(newTrigger, jobId);
      
      // Notify parent and close modal
      onTriggerCreated();
      handleClose();
    } catch (err) {
      setError(`Failed to create trigger${err instanceof Error ? `: ${err.message}` : ''}`);
    } finally {
      setSaving(false);
    }
  };
    ...
    <Modal show={show} onHide={handleClose} size="lg">
      <Modal.Header closeButton>
        <Modal.Title>
          ...
          Create Trigger {job && `for ${job.title}`}
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {loading ? (
...
export default TriggerCreateModal;