...
import { JobRunDto, JobDto, PagedResult } from '../types';
...
const Runs: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (jobId) {
        // Load runs for specific job
        const jobIdNum = parseInt(jobId);
        const [runsResult, jobResult] = await Promise.all([
          apiClient.getJobRunsByJobId(
            jobIdNum, 
            currentPage, 
            sort, 
            20,
          ...
          job ? Promise.resolve(job) : apiClient.getJob(jobIdNum)
        ]);
        ...
        // Load all runs
        const runsResult = await apiClient.getJobRuns(
          currentPage,
          sort,
          query,
      ...
      setError(`Failed to load job runs: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      setLoading(false);
    }
  }, [apiClient, jobId, currentPage, sort, query, selectedStates, showDeleted, job]);
