...
const TriggerEdit: React.FC = () => {
  ...
  // Form state
  const [triggerType, setTriggerType] = useState<'Instant' | 'Scheduled' | 'Recurring'>('Instant');
  ...
  const loadTriggerData = async () => {
    ...
    try {
      ...
      setTriggerType(triggerData.triggerType as any);
      ...
      // Type-specific data
      const triggerAny = triggerData as any;
      if (triggerAny.delayedMinutes !== undefined) {
        setDelayedMinutes(triggerAny.delayedMinutes);
      }
      ...
      if (triggerAny.definition) {
        setCronExpression(triggerAny.definition);
      }
